...
export const doclingConfig = {
  // 总开关：false 时全格式走原解析器（pandoc/pdf-parse）
  enabled: process.env.DOCLING_ENABLED === 'true',
  ...
  // 是否启用 OCR（扫描件/图片型 PDF 才需要，CPU 模式下显著增加耗时与内存）
  doOcr: process.env.DOCLING_DO_OCR === 'true',
